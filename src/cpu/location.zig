...
pub const Location = enum {
    ...
    pub fn getAddress(comptime loc: Location, cpu: *Cpu) u16 {
        return switch (loc) {
            ...
            else => @compileError("incompatible address location " ++ @tagName(loc)),
        };
    }

    pub fn get(comptime loc: Location, cpu: *Cpu) u8 {
        return switch (loc) {
            .a => cpu.regs._8.get(.a),
            .f => cpu.regs._8.get(.f),
            .b => cpu.regs._8.get(.b),
    ...
    pub fn set(comptime loc: Location, cpu: *Cpu, data: u8) void {
        switch (loc) {
