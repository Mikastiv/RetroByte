...
const Header = struct {
    ...
    pub fn print(self: *const @This()) !void {
        ...
        try stdout.print("licensee: {s}\n", .{self.licensee});
        try stdout.print("type: {s}\n", .{self.cartridge_type});

        try stdout.print("rom size: ", .{});
        try std.fmt.fmtIntSizeBin(self.rom_size * 1024).format("", .{}, stdout);
        try stdout.print("\nram size: ", .{});
        try std.fmt.fmtIntSizeBin(self.ram_size * 1024).format("", .{}, stdout);
        try stdout.print("\nregion: {s}\n", .{@tagName(self.region)});
...
var rom: []const u8 = undefined;

pub fn init(bytes: []const u8) void {
    rom = bytes;
    header = parseHeader(bytes[0x100 .. 0x100 + header_len]);
}
